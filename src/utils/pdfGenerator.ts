...
export const generateReportPDF = async (report: Report, options: { download?: boolean, email?: boolean, recipients?: string[] } = { download: true, email: true }) => {
  ...
  // Cargar fuente Geist
  // Cargar fuente Geist (Static - Local Files)
  try {
    const fontFiles = [
    ...
    const addFontToDoc = async (file: { url: string, filename: string, style: string }) => {
  ...
  // FUENTES (Usando Geist)
  const setFontBold = (size: number) => {
    doc.setFont('Geist', 'bold') // Si es variable font, bold puede requerir configuración específica, pero intentaremos standard
