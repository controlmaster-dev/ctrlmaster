...
export default function TasksPage() {
  ...
  const isAdmin =
    user?.email === "knunez@enlace.org" ||
    user?.email === "ablanco@enlace.org" ||
    user?.role === "BOSS";
...
function OperatorView({ user }: { user: any }) {
  ...
  const loadData = async (date: Date) => {
    ...
    try {
      const res = await fetch(`/api/tasks?userId=${user.id}&date=${dateStr}`, {
        cache: "no-store",
      });
  ...
  if (loading && tasks.length === 0)
    return (
      <div className="text-center p-10 font-mono text-slate-400">
        Cargando...
      </div>
    );
    ...
    <div className="max-w-5xl mx-auto space-y-8">
      <div className="space-y-6">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">Agenda Semanal</p>
        </div>
        ...
        {isSelectedDateToday && (
          <div className="space-y-2">
            <div className="flex justify-between text-xs font-bold tracking-tight text-slate-400">
      ...
      <Tabs defaultValue="pending" className="w-full">
        <TabsList className="grid w-full grid-cols-3 mb-4 bg-black/40 p-1 rounded-2xl border border-white/5 backdrop-blur-xl">
          ...
          <TabsTrigger
            value="completed"
            className="rounded-xl data-[state=active]:bg-white/10 data-[state=active]:text-emerald-500 data-[state=active]:shadow-lg data-[state=active]:backdrop-blur-md text-slate-500 hover:text-white hover:bg-white/5 transition-all duration-300"
          >
            Completadas ({tasks.filter((t) => t.status === "COMPLETED").length})
          </TabsTrigger>
          <TabsTrigger
            value="justified"
            className="rounded-xl data-[state=active]:bg-white/10 data-[state=active]:text-amber-500 data-[state=active]:shadow-lg data-[state=active]:backdrop-blur-md text-slate-500 hover:text-white hover:bg-white/5 transition-all duration-300"
          >
          ...
          {actionType !== "VIEW" && (
            <div className="space-y-4 py-2">
              <div className="bg-slate-50 dark:bg-white/5 p-3 rounded-lg text-sm text-slate-600 dark:text-slate-300">
                {actionType === "COMPLETE" &&
                  "Añade un comentario opcional sobre la ejecución."}
                {actionType === "JUSTIFY" &&
                  "Es obligatorio indicar por qué no se pudo realizar la tarea."}
...
function AdminView({ user: _user }: { user: any }) {
  ...
  const toggleDay = (d: Date) => {
    ...
    if (exists) {
      setSelectedDays(
        selectedDays.filter((sd) => format(sd, "yyyy-MM-dd") !== str)
      );
    } else {
  ...
  const getPriorityColor = (p: string) => {
    switch (p) {
      ...
      case "MEDIUM":
        return "border-amber-200 text-amber-700 bg-amber-50 dark:bg-amber-900/20 dark:text-amber-400 dark:border-amber-800";
      default:
        return "border-slate-200 text-slate-700 bg-slate-50 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700";
    }
  };

  const sortedTasks = [...tasks].sort((a, b) => {
    const dA = parseISO(a.scheduledDate);
    const dB = parseISO(b.scheduledDate);
                    ...
                    <div className="flex justify-between items-center bg-slate-50 dark:bg-white/5 p-3 rounded-lg">
                      <div className="space-y-1">
                        ...
                        <p className="text-[11px] text-slate-500">
                          Selecciona los días para asignar estas tareas. (Mostrando
                          próximas 8 semanas)
                        </p>
                      </div>
                      <div className="flex gap-2">
                    ...
                    <div className="space-y-6">
                      {Array.from({ length: 8 }).map((_, weekIndex) => {
                        ...
                        const allSelected = weekDays.every((d) =>
                          selectedDays.some(
                            (sd) =>
                              format(sd, "yyyy-MM-dd") === format(d, "yyyy-MM-dd")
                          )
                        );
                          ...
                          <div key={weekIndex} className="space-y-2">
                            <div className="flex items-center gap-2">
                              ...
                              <Button
                                ...
                                className={`h-5 text-[10px] px-2 ${
                                  allSelected ? "text-red-500" : "text-blue-500"
                                }`}
                                onClick={() => {
                                  if (allSelected) {
                                    const toRemove = weekDays.map((d) =>
                                      format(d, "yyyy-MM-dd")
                                    );
                                    setSelectedDays(
                                      selectedDays.filter(
                                    ...
                                    const existing = selectedDays.map((d) =>
                                      format(d, "yyyy-MM-dd")
                                    );
                                    const newDaysBatch = weekDays.filter(
                                      (d) =>
                                        !existing.includes(format(d, "yyyy-MM-dd"))
                                    );
                                    setSelectedDays([
                                      ...selectedDays,
                                      ...newDaysBatch,
                            ...
                            <div className="grid grid-cols-7 gap-1">
                              {weekDays.map((d) => {
                                ...
                                const isSel = selectedDays.some(
                                  (sd) => format(sd, "yyyy-MM-dd") === s
                                );
                    ...
                    {newTasks.map((t, idx) => (
                      <div key={idx} className="flex gap-2 items-start">
                        <div className="grid gap-1 flex-1">
                          <Label className="text-[10px] text-slate-400">Tarea</Label>
                          <Input
                            value={t.title}
                            onChange={(e) => {
                              const copy = [...newTasks];
        ...
        <Dialog open={isReportOpen} onOpenChange={setIsReportOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Genera y envía el reporte de rendimiento de la semana pasada
                (Lunes-Domingo) en formato PDF.
              </DialogDescription>
            </DialogHeader>
            ...
            {sortedTasks.map((task) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`p-3 rounded-xl text-slate-400 ${
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {task.reminderSent && (
                        <Badge
                          variant="outline"
                          className="bg-orange-50 text-orange-700 border-orange-200 dark:bg-orange-900/20 dark:text-orange-400 dark:border-orange-800 text-[10px] h-5 px-2"
                        >
                ...
                <div className="flex items-center gap-1">
                  <Button
                    size="icon"
                    variant="ghost"
                    className="text-slate-300 hover:text-red-500 hover:bg-red-500/10"
