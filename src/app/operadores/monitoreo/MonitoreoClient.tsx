...
export function MonitoreoClient() {
    ...
    return (
        <div className="flex flex-col min-h-screen bg-background text-muted-foreground pt-14 md:pt-0 overflow-y-auto transition-colors duration-500 relative">
            {/* Elite Monitoring Texture Overlay */}
            <div className="absolute inset-0 pointer-events-none opacity-[0.03] bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] mix-blend-overlay z-0" />
            ...
            <div className="h-10 md:h-12 bg-card/40 backdrop-blur-xl border-b border-border/60 flex items-center justify-between px-2 md:px-5 shrink-0 relative z-10">
                <div className="flex items-center gap-2 md:gap-4">
                    ...
                    <span className="text-[10px] font-medium text-foreground tracking-[0.2em] hidden md:block uppercase opacity-90">Multiview 1.0</span>
                    <span className="text-[10px] font-medium text-muted-foreground tracking-tight border-l border-border/50 pl-3 ml-1">{currentTime}</span>
                    <div className="h-4 w-[1px] bg-border mx-2 hidden md:block opacity-40" />
                    <span className="hidden md:inline-block text-[9px] text-muted-foreground/70 bg-muted/40 px-2 py-0.5 rounded-sm border border-border/40 uppercase font-medium tracking-wide">
                        PULSAR <span className="text-primary font-bold">[ESPACIO]</span> PARA CORTAR
                    </span>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    <Dialog>
                        <DialogTrigger asChild>
                            <Button variant="ghost" size="sm" className="h-8 gap-2 text-[10px] font-medium tracking-widest hover:bg-muted text-muted-foreground hover:text-foreground transition-all bg-card/50 border border-border/50 rounded-md uppercase">
                                <Activity className="w-3.5 h-3.5 text-primary/70" />
                                <span className="hidden md:inline">Estado del Sistema</span>
                            </Button>
                        </DialogTrigger>
                        <DialogContent className="max-w-6xl bg-black/40 backdrop-blur-3xl border-white/10 text-white p-0 overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.8)] rounded-3xl">
                            <MonitoringStats />
                        </DialogContent>
                    </Dialog>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 p-2 shrink-0">
                ...
                >
                    ...
                    <div className="absolute inset-0 bg-green-500/0 group-hover/master:bg-green-500/5 transition-colors z-50 flex items-center justify-center pointer-events-none">
                        <span className="opacity-0 group-hover/master:opacity-100 text-[10px] font-bold text-green-500 bg-black/80 px-4 py-1 rounded-full border border-green-500/20 tracking-tight transition-all duration-300 transform translate-y-2 group-hover/master:translate-y-0">Clic para cortar</span>
                    </div>
                </div>

                <div className="relative bg-black overflow-hidden aspect-video ring-1 ring-border">
                    <VideoJSPlayer
                        ...
                        >
                            {stream ? (
                                ...
                                <div className="w-full h-full flex items-center justify-center border-2 border-dashed border-border/20 bg-muted/10 rounded-sm">
                                    <span className="text-muted-foreground/30 font-light text-xl uppercase tracking-[0.3em]">Sin señal</span>
                                </div>
                            )}
                        </div>
                    );
