...
export async function GET(req: NextRequest) {
  try {
    ...
    console.log(
      `[Shift Reminders] Target Date: ${targetDate.toISOString().split("T")[0]}, Assigned: ${info.name}`
    );

    if (info.name === "N/A" || !info.name) {
      return NextResponse.json({
        message: "No operator assigned for this date, skipping email.",
      });
    }

    await connectMongo();
    const user = await UserModel.findOne({ name: info.name }).lean();

    if (!user || (!user.email && !user.phone && !isTest)) {
      return NextResponse.json({
        message: `No contact info found for operator ${info.name}`,
      });
    }
