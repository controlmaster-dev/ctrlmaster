...
async function generateAndSendReport(recipients, mode = 'LAST') {
  try {
    ...
    autoTable(doc, {
      ...
      didParseCell: function (data) {
        if (data.section === 'body' && data.column.index === 5) {
          ...
          if (val >= 80) data.cell.styles.textColor = [22, 163, 74]; else
            if (val >= 50) data.cell.styles.textColor = [234, 179, 8]; else
