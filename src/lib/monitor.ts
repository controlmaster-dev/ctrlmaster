...
export interface MonitorResult {
    ...
    type?: 'BLACK_SCREEN' | 'SILENCE' | 'FROZEN' | 'CRITICAL' | 'NETWORK_ERROR' | 'PARTIAL_OUTAGE'
    details?: string
    screenshotPath?: string
}

export async function checkMultiviewStatus(): Promise<MonitorResult> {
    let browser = null
    try {
        console.log("[Monitor] Launching Puppeteer...")
        // Launch headless browser
        // Args needed for VPS/Docker environments usually
        ...
        if (emailInput) {
            ...
            // Use xpath selector strategy (Puppeteer v24+)
            ...
            if (button) {
                ...
                await Promise.all([
                    page.waitForNavigation({ waitUntil: 'networkidle2', timeout: 15000 }).catch(e => console.log("Nav timeout ignored", e)), // Added e usage to avoid unused var if flexible
        ...
        // Since we can't pass complex functions to evaluate easily across frames, we will use page.frames()

        let totalVideos = 0
        const allStatuses: { label: string, isPlaying: boolean, hasError: boolean, hasAudio: boolean }[] = []

        // Scrape simple list of labels in DOM order (1-to-1 match with frames assumption)
        const labelsList = await page.evaluate(() => {
            const iframes = Array.from(document.querySelectorAll('iframe'))
            return iframes.map((iframe, index) => {
                let label = `Canal ${index + 1}`
                let wrapper = iframe.parentElement as HTMLElement | null
                for (let i = 0; i < 4; i++) {
                    if (!wrapper) break
                    ...
                    if (candidate) {
                        label = candidate.substring(0, 30) // Truncate
                        break
                    }
        ...
        for (const frame of frames) {
            try {
                // Skip main frame logic alignment for simplicity? 
                // We'll increment iframeIndex only if frame != mainFrame
                ...
                const frameResult = await frame.evaluate((lbl) => {
                    ...
                    const statuses = docVideos.map((v, index) => {
                        ...
                        // Fallback numbering if generic "Canal X" was passed or scraping failed
                        if (finalLabel.startsWith('Canal ')) {
                            // Check if it's generic "Canal N"
                            // Actually, if labelsList has "Canal 1", "Canal 2"... 
                            // We should respect that OR use startId.
                            // Let's rely on labelsList. If it failed to find text, it returns Canal N (DOM index).
                            // That is semantically correct (Canal 1, Canal 2...).
                ...
                if (frameResult) {
                    ...
                    // If multiple videos found, global counter increases
                ...
                // Ignore cross-origin frame access errors, but still increment iframeIndex if it's an iframe
        ...
        // Generate Full List Report
        const lines = allStatuses.map(s => {
            let statusText = ''
            if (s.hasError) statusText = 'Error de Señal'
            else if (!s.isPlaying) statusText = 'Congelado/Video Detenido'
            else statusText = 'Imagen OK'
        ...
        // Dallas Exception: If only Dallas fails, it's just a WARNING or OK (based on user request to 'evade').
        // But user wants "Reporte Diario", so we return OK implies "Normal" but with details.
        ...
        // Always return the full detailed list
        return {
            status,
