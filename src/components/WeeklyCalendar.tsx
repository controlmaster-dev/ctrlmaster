...
export function WeeklyCalendar({ operators, onUpdateSchedule = undefined, currentWeekStart, onWeekChange, isLoading = false, onEditUser = undefined }) {
                ...
                <div key={shift.tempId} className="bg-muted/30 p-5 rounded-2xl border border-border space-y-5 relative group shadow-sm">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select value={shift.targetOpId} onValueChange={(v) => updateEditingShift(shift.tempId, 'targetOpId', v)}>
                      <SelectTrigger className={`h-10 text-sm border-border rounded-md ${shift.targetOpId !== editingState.originalOpId ? 'bg-amber-500/10 text-amber-500 border-amber-500/20 font-medium' : 'bg-card text-foreground'}`}>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent className="bg-card border-border">
                        {operators.map((op) => (
