...
export function ReportDetailModal({ isOpen, onClose, report, currentUser, onUpdate }: ReportDetailModalProps) {
    ...
    React.useEffect(() => {
        if (isOpen && report) {
            ...
            // 2. Register View (Fire and forget)
            if (currentUser) {
                ...
                <div className="w-full h-full overflow-y-auto pb-32 md:p-6 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-border [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-muted-foreground/50">
                        ...
                        <div className="flex items-center gap-4 pr-2">
                            ...
                            <div className="flex-1 min-w-0">
                                <DialogTitle className="text-lg md:text-2xl font-bold flex items-center gap-4 truncate text-foreground">
                                    ...
                                    <Badge variant="outline" className="hidden sm:inline-flex font-normal border-border text-muted-foreground">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2 p-4 md:p-0">
                        ...
                        <div className="flex flex-col gap-6">
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                                    ...
                                    <span className="font-medium text-foreground">{displayReport.operatorName}</span>
                                </div>
                                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                                ...
                                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                                    ...
                                    <span className="capitalize">{displayReport.status === 'resolved' ? 'Resuelto' : displayReport.status}</span>
                                </div>
                            </div>
                            ...
                            <div className="border-t pt-4 border-border">
                                {loading ? (
                                    <div className="text-center py-4 text-muted-foreground text-xs">Cargando comentarios...</div>
                                ) : (
                                    <ReportSocials
                        ...
                        <div className="bg-muted/30 p-3 rounded-lg border border-border h-full">
                            <h4 className="text-xs font-bold text-muted-foreground mb-2">Descripción del problema</h4>
                            ...
                            {displayReport.attachments && displayReport.attachments.length > 0 && (
                                <div className="mt-4 pt-4 border-t border-border">
                                    <h4 className="text-xs font-bold text-muted-foreground mb-2">Adjuntos</h4>
                                    <div className="flex gap-2 flex-wrap">
                                        {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                                        {displayReport.attachments.map((file: any, idx: number) => (
                                            <div key={idx} className="relative w-24 h-24 bg-card rounded-lg overflow-hidden border border-border flex items-center justify-center group cursor-pointer hover:border-ring transition-colors" onClick={() => {
                                                ...
                                                {file.type === 'IMAGE' ? (
                                                    <img src={file.data || file.url} alt="Adjunto" className="w-full h-full object-cover transition-transform group-hover:scale-105" />
                                                ) : (
                    ...
                    {displayReport.views && displayReport.views.length > 0 && (
                        <div className="mt-4 flex items-center gap-2 text-[10px] text-muted-foreground">
                            ...
                            {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                            <span>Visto por: {displayReport.views.map((v: any) => v.user.name).join(', ')}</span>
                        </div>
                    )}
                </div>
            </DialogContent>
