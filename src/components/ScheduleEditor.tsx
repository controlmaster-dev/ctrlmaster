...
export function ScheduleEditor({ value, onChange }: ScheduleEditorProps) {
  ...
  const updateShift = (index: number, field: string, val: number | number[]) => {
    ...
    newShifts[index] = { ...newShifts[index], [field]: val } as Shift;
    setShifts(newShifts);
    onChange(newShifts);
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center px-1">
        <div className="flex flex-col">
          ...
          <p className="text-[10px] text-muted-foreground/50 font-medium">
            Define los periodos de actividad semanal
          </p>
        </div>
        <Button
          type="button"
      ...
      {shifts.length === 0 && (
        <motion.div
          ...
          className="text-[11px] text-muted-foreground font-semibold uppercase tracking-widest text-center py-10 border-2 border-dashed border-border rounded-2xl bg-muted/20"
        >
        ...
        <AnimatePresence mode="popLayout">
          {shifts.map((shift, index) => (
            <motion.div
              ...
              className="bg-card border border-border p-6 rounded-2xl relative group shadow-sm"
            >
              <Button
                variant="ghost"
                size="icon"
              ...
              <div className="grid gap-6">
                <div className="space-y-3">
                  <Label className="text-[10px] font-semibold text-muted-foreground uppercase tracking-widest ml-1 opacity-50">
                    Días de actividad
                  </Label>
                ...
                <div className="flex items-center gap-8">
                  <div className="flex-1 space-y-3">
                    <Label className="text-[10px] font-semibold text-muted-foreground uppercase tracking-widest ml-1 opacity-70">
                      Hora de Inicio
                    </Label>
                    <Select
                      value={String(shift.start)}
                      onValueChange={(v) =>
