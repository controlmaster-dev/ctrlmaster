...
export function EmailSendModal({
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[500px] bg-[#18181B] border-white/10 text-slate-200">
                <DialogHeader>
                    ...
                    <DialogDescription className="text-slate-400">
                        Configura los destinatarios para el reporte <span className="text-white font-mono font-bold">#{reportId.slice(0, 6)}</span>.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="py-4 space-y-6">
                    ...
                    <div className="flex items-start gap-3 p-4 rounded-lg border border-white/5 bg-white/[0.02]">
                        <Checkbox
                            ...
                            className="mt-1 data-[state=checked]:bg-[#FF0C60] border-slate-600"
                        />
                        <div className="grid gap-1.5 leading-none">
                            <Label htmlFor="defaults" className="text-sm font-medium text-white cursor-pointer">
                    ...
                    <div className="space-y-3">
                        <Label className="text-xs font-bold uppercase text-slate-500 tracking-wider">Otros Destinatarios</Label>
                        <div className="flex gap-2">
                            <Input
                                value={inputValue}
                                onChange={(e) => setInputValue(e.target.value)}
                                placeholder="ejemplo@enlace.org"
                                className="bg-transparent border-white/10 text-slate-200 focus-visible:ring-[#FF0C60]"
                        ...
                        {customEmails.length > 0 && (
                            <div className="flex flex-wrap gap-2 mt-2">
                                {customEmails.map(email => (
                                    <div key={email} className="flex items-center gap-1 pl-3 pr-1 py-1 rounded-full bg-white/5 border border-white/10 text-xs text-slate-300">
                                        <span>{email}</span>
                                        <button onClick={() => handleRemoveEmail(email)} className="p-1 hover:text-white rounded-full hover:bg-white/10">
                                            <X className="w-3 h-3" />
                                        </button>
                                    </div>
                                ))}
