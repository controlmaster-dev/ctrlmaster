...
const DEFAULT_RECIPIENTS = EMAIL_CONFIG.DEFAULT_RECIPIENTS;
      ...
      <DialogContent className="sm:max-w-[500px] bg-background border-border text-foreground">
        <DialogHeader>
          ...
          <DialogDescription className="text-muted-foreground">
            Configura los destinatarios para el reporte{" "}
        ...
        <div className="py-4 space-y-6">
          ...
          <div className="space-y-3">
            <Label className="text-xs font-bold text-muted-foreground tracking-tight">
              Otros destinatarios
            </Label>
            ...
            {customEmails.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-2">
                {customEmails.map((email) => (
                  <div
                    key={email}
                    className="flex items-center gap-1 pl-3 pr-1 py-1 rounded-full bg-muted border border-border text-xs text-foreground"
                  >
                    <span>{email}</span>
                    <button
