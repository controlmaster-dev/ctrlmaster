...
export function VideoJSPlayer({
      ...
      player.ready(() => {
        ...
        const setupSmartMeter = () => {
          ...
          const loop = (timestamp: number) => {
            ...
            const isUnmuted = !player.muted() && (player.volume() ?? 0) > 0;
  ...
  return (
    <div
      className={`relative w-full h-full group bg-black overflow-hidden border-[2px] transition-all duration-500 rounded-sm ${
        isProgram
    ...
    >
      {(isProgram || isPreview) && (
        <div
          className={`absolute inset-0 pointer-events-none z-10 border-[1px] opacity-20 ${
            isProgram ? "border-red-500" : "border-green-500"
          }`}
        />
      )}

      <div
        className={`absolute top-0 left-0 z-40 px-2.5 py-1 ${
          isProgram
            ? "bg-red-600"
            : isPreview
            ? "bg-green-600"
      ...
      >
        <span
          className={`text-[9px] font-bold tracking-[0.2em] uppercase ${
            isProgram || isPreview ? "text-white" : "text-foreground/90"
          }`}
        >
          {channelLabel || title}
        </span>
      ...
      {(isProgram || isPreview) && (
        <div
          className={`absolute top-0 right-0 z-40 px-2.5 py-1 text-[9px] font-bold text-white tracking-widest ${
            isProgram ? "bg-red-600" : isPreview ? "bg-green-600" : ""
          }`}
        >
      ...
      <div
        ref={meterRef}
        className={`absolute bottom-2 right-2 z-30 flex gap-[3px] bg-black/60 p-1.5 rounded-sm border border-white/5 shadow-2xl backdrop-blur-md pointer-events-none ${
          variant !== "default" ? "h-36 px-3" : "h-24 px-2"
        }`}
      >
        <div className={variant !== "default" ? "w-3" : "w-2"}>
          {Array.from({ length: 24 }).map((_, i) => (
            <div
              key={i}
              className={`w-full h-[3.8%] mb-[1px] transition-opacity duration-300 ${
                i > 20 ? "bg-red-500" : i > 15 ? "bg-yellow-400" : "bg-green-500"
              }`}
              style={{ opacity: 0.2 }}
            />
          ))}
