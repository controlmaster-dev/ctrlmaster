...
export function SpecialEventsManager() {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            {isCreating && (
                <Card className="bg-[#121214] border-white/10 mb-6">
                    <CardHeader><CardTitle className="text-lg">Crear Evento</CardTitle></CardHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {events.map(event => (
                    <Card key={event.id} className="bg-white/5 border-white/5 hover:border-white/10 transition-all group relative overflow-hidden">
                        <div className={`absolute top-0 left-0 w-1 h-full ${event.isActive ? 'bg-[#FF0C60]' : 'bg-slate-700'}`} />
                        <CardHeader className="pb-2">
                            <div className="flex justify-between items-start">
                                <Badge variant="outline" className={`${event.isActive ? 'text-[#FF0C60] border-[#FF0C60]/30 bg-[#FF0C60]/10' : 'text-slate-500 border-white/10'}`}>
                                    {event.isActive ? 'Activo' : 'Inactivo'}
                                </Badge>
                                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <Button variant="ghost" size="icon" onClick={() => handleToggleActive(event)} className="h-8 w-8 hover:bg-white/10" title={event.isActive ? "Desactivar" : "Activar"}>
                                        <CalendarIcon className="w-4 h-4" />
                                    </Button>
                                    <Button variant="ghost" size="icon" onClick={() => handleDeleteEvent(event.id)} className="h-8 w-8 text-rose-500 hover:bg-rose-900/20">
                        ...
                        <CardContent>
                            <div className="flex justify-between items-center mt-4">
                                ...
                                <Button onClick={() => openEventSchedule(event)} className="bg-white/10 hover:bg-[#FF0C60] text-white border border-white/20 hover:border-[#FF0C60]">
                                    Gestionar Horarios
                                </Button>
                            </div>
                        </CardContent>
