...
import { jsx as _jsx, jsxs as _jsxs } from "react/jsx-runtime"; export function BitcentralWidget({ users, isReadOnly = false }: { users: Array<{ id: string; name: string }>; isReadOnly?: boolean }) {
              ...
              className: "flex flex-col gap-2", children:
                isLoading ?
                  ...
                  days.map((date) => {
                    ...
                    return (
                      _jsxs("div", {
                        className: `group flex items-center gap-3 p-3 transition-all border-l-[3px] ${isToday ? 'bg-blue-500/5 border-blue-500 shadow-inner' : 'hover:bg-muted/30 border-transparent hover:border-border'} ${isReadOnly ? 'cursor-default' : 'cursor-pointer'}`,
                        onClick: () => !isReadOnly && setSelectedDate(date), children: [
