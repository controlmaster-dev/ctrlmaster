...
export function ActiveUsersWidget({ users }: { users: ActiveUser[] }) {
              ...
              activeUsers.map((user) => (
                ...
                >
                  <div className="relative">
                    ...
                    <span className="absolute -bottom-0.5 -right-0.5 w-2 h-2 bg-emerald-500 border-2 border-background rounded-full shadow-sm" />
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-[10px] font-medium text-muted-foreground truncate tracking-tight">
                        {user.currentPath === "/"
                          ? "Inicio"
                          : user.currentPath?.replace("/", "") || "Navegando"}
                      </span>
                    </div>
                  </div>
                </div>
