...
import { Upload, X, Check, Mail, Settings } from "lucide-react";
import { cn } from "@/lib/utils";
import { ReportFormData } from "@/hooks/useReportForm";

interface EvidenceStepProps {
  formData: Pick<ReportFormData, 'attachments' | 'isResolved' | 'dateResolved' | 'sendEmail' | 'emailRecipients'>;
            ...
            {formData.attachments.map((file, idx) => (
              <div key={idx} className="flex items-center gap-4 p-4 rounded-xl bg-card border border-border group hover:border-[#ff3366]/20 transition-all">
                <div className="w-12 h-12 rounded-lg bg-muted shrink-0 overflow-hidden ring-1 ring-border shadow-inner">
                  <img src={(file.data as string) || file.url} className="w-full h-full object-cover filter contrast-125" alt="Evidence" />
                </div>
